...
**D4.** Introduce an argv-based command runner in `src/lib/cli.ts` (built on `node:child_process.execFile`, no shell) and route both openers through it. *Amended 2026-09-08 at the merge with `main`:* `main` landed its own argv runner, `run`, in the same file for the same reason (its archived `SHELL-ARGV-SAFETY-PLAN.md`), so this one survives beside it as `runCapturing` — the capturing sibling that resolves rather than rejects on a non-zero exit, which is what reading Herdr's stderr error envelope needs. The editor branch uses `main`'s `run`. *Rejected: keeping `exec` and hand-quoting the path*, because hand-quoting is what produced the current defect and still fails on a path containing a quote. **The unquoted-path defect is fixed here, in Phase 1 of this plan, not deferred to `/orchestrate`** — the Herdr opener needs the same helper, and landing the fix twice is worse than landing it once. Scope is limited to `openWorktreePath`; the other unquoted interpolations in `src/lib/git.ts` are out of scope (see §8).
...
- ~~**Whether `herdr status server --json` is the right liveness probe**, or whether a failed `worktree open` should be the only signal — the probe costs an extra process spawn on every open.~~ **Answered 2026-09-08 at Phase 4** (F-040): it is not. The probe is gone and a failed `worktree open` is the only liveness signal, which is also the only one that carries Herdr's `code` and `message`. See the amended §4.3.
...
- ~~**Whether agent start should be awaited or fire-and-forget.** D9 makes it opt-in, which caps the blast radius, but a 30 s ceiling on an awaited call is still long for a command whose real work is already done.~~ **Answered 2026-09-08 at Phase 5: awaited, with the ceiling halved.** Fire-and-forget was rejected because the failure this feature actually expects is a name collision (§5), which Herdr reports *after* the call — a fire-and-forget start would swallow exactly the error worth printing. `AGENT_START_TIMEOUT_MS` is 15 s (`src/integrations/herdr.ts`), inside the schema's `(3000, 300000]` and half Herdr's own default; erring long is deliberate, because a start that times out on an agent which did come up warns about an agent that is in fact running. **Note the limit:** `--timeout` bounds Herdr's readiness wait, not the subprocess this repo spawned, so a `herdr` client that accepts and never answers still hangs. That half is F-041 and is still open.
