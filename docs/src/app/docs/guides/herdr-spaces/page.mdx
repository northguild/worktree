...
declined, a removal that failed, a worktree `cleanup` held back for uncommitted changes or a live
agent — none of those close anything. Their checkouts are still on disk, and a space with no window
